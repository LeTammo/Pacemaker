...
export default function SwimsHub() {
    ...
    return (
        <main className="p-6 md:p-10 max-w-6xl mx-auto space-y-6 w-full">
            <div>
                <h1 className="text-2xl font-bold text-white">Swimming</h1>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl">
                    <p className="text-xs text-zinc-300 uppercase">Total swims</p>
                    <p className="text-2xl font-bold text-white">{stats?.total_swims ?? 0}</p>
                </div>
                <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl">
                    <p className="text-xs text-zinc-300 uppercase">Avg swim distance</p>
                    <p className="text-2xl font-bold text-white">{stats?.average_swim_distance_km ?? 0} km</p>
                </div>
                <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl">
                    <p className="text-xs text-zinc-300 uppercase">Swims / week</p>
                    <p className="text-2xl font-bold text-white">{stats?.average_swims_per_week ?? 0}</p>
                </div>
            </div>

            {isLoading ? (
                <p className="text-zinc-300 text-sm">Fetching swim sessions...</p>
            ) : (
                <div className="mt-6">
